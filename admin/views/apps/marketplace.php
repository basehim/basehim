...
<div class="flex items-center justify-between mb-5 flex-wrap gap-3">
    <div>
        <h2 class="text-xl font-semibold text-slate-900">App Marketplace</h2>
        <p class="text-sm text-slate-500">Browse and install apps from the Basehim marketplace.</p>
    </div>
    <a href="<?= $base ?>/admin/apps" class="inline-flex items-center gap-2 px-3 py-2 border border-slate-200 rounded-lg text-sm text-slate-600 hover:bg-slate-50">
        <?= icon('arrow-left', 'w-4 h-4') ?> Back to Apps
    </a>
</div>
...
<div class="bg-white rounded-xl border border-slate-200 p-3 mb-4 flex items-center gap-2 flex-wrap">
    <div class="relative flex-1 min-w-[220px]">
        ...
        <input id="mk-q" type="text" placeholder="Search apps…" class="w-full pl-10 pr-3 py-2 border border-slate-300 rounded-lg text-sm outline-none focus:ring-2 focus:ring-teal-200 focus:border-teal-500">
    </div>
    <select id="mk-sort" class="px-3 py-2 border border-slate-300 rounded-lg text-sm outline-none focus:border-teal-500">
    ...
    <aside class="lg:col-span-1 space-y-5">
        <div class="bg-white rounded-xl border border-slate-200 p-4">
            ...
            <div id="mk-cats" class="space-y-1 text-sm">
                <button data-cat="" class="mk-cat block w-full text-left px-2 py-1.5 rounded-lg bg-teal-50 text-teal-700 font-medium active">All apps</button>
            </div>
        </div>
        <div class="bg-white rounded-xl border border-slate-200 p-4">
            <h3 class="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-2">Popular tags</h3>
            <div id="mk-tags" class="flex flex-wrap gap-1.5"><span class="text-xs text-slate-400">Loading…</span></div>
        </div>
    </aside>

    <div class="lg:col-span-3">
        <div id="mk-status" class="hidden mb-4"></div>
        <div id="mk-grid" class="grid grid-cols-1 sm:grid-cols-2 gap-4">
...
<style>
.mk-cat.active { background:#ccfbf1; color:#0f766e; font-weight:600; }
.mk-tag.active { background:#14b8a6; color:#fff; border-color:#14b8a6; }
...
(function () {
    ...
    function loadFacets() {
        get('/admin/apps/marketplace/facets.json').then(function (d) {
            if (!d || !d.ok) return;
            var cats = $('mk-cats');
            ...
            cats.addEventListener('click', function (e) {
                ...
                cats.querySelectorAll('.mk-cat').forEach(function (x) { x.classList.remove('active','bg-teal-50','text-teal-700','font-medium'); });
            ...
            (d.tags || []).forEach(function (t) {
                var b = document.createElement('button');
                b.className = 'mk-tag text-xs px-2 py-1 rounded-full border border-slate-200 text-slate-600 hover:border-teal-400';
                b.dataset.tag = t.name; b.textContent = t.name;
                tags.appendChild(b);
            });
            ...
            tags.addEventListener('click', function (e) {
                ...
                else { tags.querySelectorAll('.mk-tag').forEach(function (x) { x.classList.remove('active'); }); btn.classList.add('active'); state.tag = btn.dataset.tag; }
                state.page = 1; load();
            });
        });
    }
    ...
    function load() {
        ...
        get('/admin/apps/marketplace/browse.json?' + qs).then(function (d) {
            if (!d || !d.ok) {
    ...
    function renderGrid(apps) {
        var grid = $('mk-grid');
        if (!apps.length) {
            grid.innerHTML = '<div class="col-span-full text-center text-slate-400 py-16"><?= icon('puzzle-piece', 'w-8 h-8 mb-2 block mx-auto opacity-40') ?>No apps match your search.</div>';
            return;
        }
        grid.innerHTML = apps.map(function (t) {
            var icon = t.icon_url
                ? '<img src="' + esc(t.icon_url) + '" alt="" class="w-12 h-12 rounded-lg object-cover bg-slate-100 shrink-0" onerror="this.style.display=\'none\'">'
                : '<div class="w-12 h-12 rounded-lg grid place-items-center bg-gradient-to-br from-teal-50 to-emerald-100 text-teal-500 shrink-0"><?= icon('puzzle-piece', 'w-5 h-5') ?></div>';
            var feat = t.featured == 1 ? '<span class="text-[10px] px-1.5 py-0.5 rounded-full bg-amber-100 text-amber-700 font-bold"><?= icon('star', 'w-4 h-4 mr-0.5') ?>Featured</span>' : '';
            ...
            if (t.installed && t.installed_version === t.version) {
                action = '<button disabled class="w-full px-3 py-2 bg-slate-100 text-slate-400 rounded-lg text-sm font-medium"><?= icon('check', 'w-4 h-4 mr-1') ?>Installed' + (t.active ? ' & active' : '') + '</button>';
                if (!t.active) note = '<div class="text-[11px] text-amber-600 mt-1 text-center">Activate it on the Apps page</div>';
            } else if (t.installed) {
                ...
                action = '<button class="mk-install w-full px-3 py-2 bg-teal-600 hover:bg-teal-700 text-white rounded-lg text-sm font-medium" data-slug="' + esc(t.slug) + '"><?= icon('arrow-down-tray', 'w-4 h-4 mr-1') ?>Install</button>';
            }
            ...
            return '<div class="bg-white rounded-xl border border-slate-200 p-4 flex flex-col">'
                ...
                + '<div class="mt-auto">' + action + note + '</div>'
                + '</div>';
        }).join('');

        grid.querySelectorAll('.mk-install').forEach(function (b) { b.addEventListener('click', function () { install(b); }); });
    }

    function install(btn) {
        var slug = btn.dataset.slug, original = btn.innerHTML;
        btn.disabled = true;
        ...
        fetch(BASE + '/admin/apps/marketplace/install', { method: 'POST', body: fd, credentials: 'same-origin', headers: { 'Accept': 'application/json' } })
            ...
            .then(function (d) {
                if (d.ok) {
                    ...
                    showStatus('<?= icon('check-circle', 'w-4 h-4 mr-1') ?>"' + esc(d.manifest && d.manifest.name || slug) + '" ' + (d.replaced ? 'updated' : 'installed') + '. <a href="' + BASE + '/admin/apps" class="underline font-medium">Go to Apps</a> to activate it.', 'ok');
                } else {
